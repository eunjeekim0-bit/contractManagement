...
화면을 **직접 수정하면서 바로 확인**하고 싶을 때 사용합니다.
파일을 저장하는 순간 브라우저가 자동으로 새로고침됩니다. (Python 만 있으면 되고, 추가 설치는 필요 없습니다.)

**실행**

- **Windows**: `serve.bat` 더블클릭
- **macOS / Linux**: 터미널에서 `./serve.sh`

실행하면 브라우저가 자동으로 열립니다 → **http://localhost:5500**
종료하려면 실행 창에서 `Ctrl + C` 를 누르세요.

**작업 방법**

1. `serve.bat` / `serve.sh` 로 서버를 켠 상태에서
2. `mockup/` 안의 `.html` · `.css` · `.js` 파일을 편집기(VS Code, 메모장 등)로 열어 수정하고 저장
3. 브라우저가 자동으로 새로고침되어 변경 내용이 바로 보입니다

> **데이터 수정**: 화면에 보이는 계약/의무/조직 데이터는 `data/*.json` 이 원본입니다.
> 서버를 켠 상태에서 `data/*.json` 을 수정·저장하면 `mockup/data.js` 가 자동으로 다시 생성되고
> 브라우저도 새로고침됩니다. (수동으로 `data.js` 를 직접 고칠 필요가 없습니다.)

> 파일을 그냥 더블클릭해서 여는 방식(위의 “목업 확인하기”)과의 차이는 **자동 새로고침**뿐입니다.
> 화면 결과물은 동일합니다.
